def build_evidence(alert, threat_intelligence, risk_assessment, classification):
    """
    Build evidence-based incident context.

    Separates confirmed evidence from interpretations
    ...
    if mitre_ids:
        confirmed.append(
            f"MITRE techniques associated with the alert: "
            f"{', '.join(mitre_ids)}."
        )
